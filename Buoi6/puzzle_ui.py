...
class PuzzleUI:
    ...
    def setup_ui_layout(self):
        ...
        # Section: Max Depth
        depth_frame = tk.Frame(left_panel, bg=BG_CARD)
        depth_frame.pack(fill="x", padx=15, pady=10)
        
        depth_lbl = tk.Label(depth_frame, text="Độ sâu giới hạn (Max Depth):", font=("Segoe UI", 9), fg=COLOR_TEXT, bg=BG_CARD)
        depth_lbl.pack(side="left", padx=(0, 10))
        ...
        # Log box label
        log_lbl = tk.Label(right_panel, text="Nhật ký tìm kiếm (IDS Logs):", font=("Segoe UI", 9, "bold"), fg=COLOR_TEXT, bg=BG_CARD)
    ...
    def start_solve_thread(self):
        ...
        try:
            ...
            messagebox.showerror("Cấu hình lỗi", "Vui lòng nhập độ sâu giới hạn hợp lệ!")
            return
        ...
        t = threading.Thread(target=self.run_solver, args=(start_state, max_depth))
        t.daemon = True
        t.start()

    # Redirect logs and run algorithm
    def run_solver(self, start_state, max_depth):
        ...
        try:
            result = self.solve_function(start_state, self.goal, max_depth)
    ...
    def on_solver_complete(self, result, elapsed_time, start_state):
        ...
        if result is not None:
            ...
            self.stats_labels["nodes"].config(text="-")
            self.step_label.config(text="Bước: 0 / 0")
            messagebox.showinfo("Thông báo", "Không tìm thấy đường đi trong giới hạn độ sâu được cấu hình!")
